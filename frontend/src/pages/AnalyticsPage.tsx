...
const AnalyticsPage = () => {
    ...
    const calculateWeeklyActivity = (papers: any[], chats: any[]) => {
        ...
        const wStats = new Array(7).fill(null);
        ...
        const allDatesSet = new Set([...papers, ...chats].map(item => new Date(item.uploaded_at || item.created_at).toDateString()));
        ...
        [...papers, ...chats].forEach(item => {
            ...
            d1.setHours(0, 0, 0, 0);
            const daysDiff = Math.round((d1.getTime() - monday.getTime()) / (1000 * 60 * 60 * 24));

            if (daysDiff >= 0 && daysDiff < 7) {
                weekActivity[daysDiff]++;
                wStats[daysDiff].activityCount++;
                wStats[daysDiff].activeHours.add(itemDate.getHours());
            }
        });

        // Calculate streak for each day
        wStats.forEach(stat => {
            if (allDatesSet.has(stat.date.toDateString())) {
                let streak = 1;
                let checkDate = new Date(stat.date);
                for (let i = 0; i < 365; i++) {
                    checkDate.setDate(checkDate.getDate() - 1);
                    if (allDatesSet.has(checkDate.toDateString())) {
                        streak++;
                    } else {
                        break;
                    }
            ...
            >
                ...
                <div className="grid grid-cols-7 gap-3">
                    {weeklyStats.length > 0 ? weeklyStats.map((stat, i) => {
                        const isActive = stat.activityCount > 0;
                        const hoursWorked = stat.activeHours.size;
                        const percentage = Math.min((hoursWorked / 8) * 100, 100);

                        return (
                            <div key={i} className="text-center">
                                <p className="text-sm text-white font-medium mb-1">{stat.dayName}</p>
                                <p className="text-[10px] text-gray-500 mb-2">{stat.fullDate}</p>
                                <div
                                    className={`h-40 rounded-xl border transition-all ${isActive
                                        ? 'bg-primary/20 border-primary/40 hover:border-primary/60'
                                        : 'bg-white/5 border-white/10 hover:border-white/20'
                                        }`}
                                    title={`${stat.activityCount} tasks, ${hoursWorked} hours`}
                                >
                                        ...
                                        {isActive && (
                                            <div className="w-full text-center space-y-1 mb-2">
                                                <div className="text-[10px] text-orange-400 font-bold flex items-center justify-center gap-1">
                                                    <Flame className="w-3 h-3" /> {stat.streak}
                                                </div>
                                                <div className="text-[10px] text-primary">{percentage.toFixed(0)}%</div>
                                            </div>
                                        )}
                                        <div
                                            className={`w-full mx-auto rounded-lg ${isActive ? 'bg-primary' : 'bg-gray-600'}`}
                                            style={{ height: `${isActive ? Math.max(percentage, 15) : 10}%`, transition: 'height 0.5s ease-out' }}
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        <div key={day} className="text-center">
                            <p className="text-sm text-white font-medium mb-1">{day}</p>
                            <p className="text-[10px] text-gray-500 mb-2">Loading...</p>
                            <div className="h-40 rounded-xl border bg-white/5 border-white/10" />
                        </div>
                    ))}
                </div>
            </motion.div>
