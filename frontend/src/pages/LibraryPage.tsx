...
const LibraryPage = () => {
    ...
    const categories = ['all', ...Array.from(new Set(papers.map(p => p.category).filter(Boolean) as string[]))];
    const allTags = Array.from(new Set(papers.flatMap(p => p.tags?.split(',').map(t => t.trim()) || []))).filter(Boolean) as string[];
            ...
            {filteredPapers.length === 0 && (
                <div className="text-center py-16">
                    ...
                    <h3 className="text-xl font-semibold text-white mb-2">
                        {searchQuery ? "Paper not found" : "No Papers Yet"}
                    </h3>
                    <p className="text-gray-400">
                        {searchQuery ? "We couldn't find any papers matching your query." : "Upload your first research paper to get started"}
                    </p>
                </div>
            )}
        </div>
